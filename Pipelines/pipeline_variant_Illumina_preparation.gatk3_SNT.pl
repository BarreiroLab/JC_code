...
#AUGUST 8th 2014
#  - Add Truseq Exome Kit
#AUGUST  11 2014
#  - Changed GATK3 version to 3.2-2
#  - Changed mapper for the abundant seq retrival for STAR
##
#AUGUST 20th 2014
#  - Added an option to build two sets of files (without SNT option)
#
#February 4th 2014
#  -Changed GATK version to 3.3-0 
...
if(exists($args{'star'}) && exists($args{'noabund'})) {
	$mem=46;
}
...
my $dbsnp_hg19_138_1000G = "/RQusagers/$usr/barreiro_group/references/hg19/dbSNP/dbsnp_138.hg19.dbsnp.noCHR.vcf";
...
my $indel_1000G_138_CEU = "/exec5/GROUP/barreiro/grenier2/references/hg19/CEU/Intervals/1000G_phase1.indels.b37.vcf";
...
my $hg19_bed_truseq = "/RQusagers/$usr/barreiro_group/references/hg19/Exome/TruSeq_exome_targeted_regions.hg19.sort.merge.bed";
...
my $abundSeqStar = "/RQusagers/$usr/barreiro_group/references/hg19/AbundantSeq/STAR";
my $hiMem = 0;

#my $intervals_hg18 = '/data/reference/hg18/GATK_dbSNP_ref/dbsnp_132.hg18.tibo.vcf.intervals';
my $intervals_hg19 = '/exec5/GROUP/barreiro/grenier2/references/hg19/Intervals/hg19_1000Genomes_indels.intervals';
my $intervals_hg19CEU = '/exec5/GROUP/barreiro/grenier2/references/hg19/CEU/Intervals/hg19CEU_1000Genomes_indels.intervals';
...
my $gatk3 = "java -Djava.io.tmpdir=\$LSCRATCH -Xmx$mem"."g -jar /home/apps/Logiciels/GATK/3.2-2/GenomeAnalysisTK.jar -l INFO";
my $gatk3_real = "java -Djava.io.tmpdir=\$LSCRATCH -Xmx$mem"."g -jar /home/apps/Logiciels/GATK/3.2-2/GenomeAnalysisTK.jar";

my $gatk3_3 = "java -Djava.io.tmpdir=\$LSCRATCH -Xmx$mem"."g -jar /home/apps/Logiciels/GATK/3.3-0/GenomeAnalysisTK.jar -l INFO";
my $gatk3_3_real = "java -Djava.io.tmpdir=\$LSCRATCH -Xmx$mem"."g -jar /home/apps/Logiciels/GATK/3.3-0/GenomeAnalysisTK.jar";
...
my $star_path = "/exec5/GROUP/barreiro/barreiro/barreiro_group/Programs/STAR_2.3.1z15/STAR";
my $bamUtil = "/exec5/GROUP/barreiro/barreiro/barreiro_group/bin/bamUtil_1.0.2/bamUtil/bin/bam";
...
if( $args{'reference'} eq 'hg19'){
	...
	elsif(lc($args{'kit'}) eq 'truseq'){
                $kit_bed = $hg19_bed_truseq;
        }
...
if($noAbund == 1 && $mito == 0){
	...
	if($star == 1){
		$cmd = "$star_path --genomeDir $abundSeqStar --readFilesIn $args{'fastq1'} $args{'fastq2'} --runThreadN $cpu --readFilesCommand zcat --outFileNamePrefix $bam_name.abundRef. --outFilterIntronMotifs RemoveNoncanonicalUnannotated --outFilterMismatchNmax 4 ";
		print "\t$cmd\n";
                print `$cmd` unless $noexec ==1;
		
		$cmd = "samtools view -bS -F4 -o $args{'root_dir'}/$bam_name.abundRef.bam $args{'root_dir'}/$bam_name.abundRef.Aligned.out.sam";
                print "\t$cmd\n";
                print `$cmd` unless $noexec ==1;

                $cmd = "rm $args{'root_dir'}/$bam_name.abundRef.Aligned.out.sam";
                print "\t$cmd\n";
                print `$cmd` unless $noexec ==1;
		...
		$cmd = "samtools view $args{'root_dir'}/$bam_name.abundRef.sort.bam | awk '{print \$1}' > $args{'root_dir'}/$bam_name.ids_to_rem.txt";
                print "\t$cmd\n";
                print `$cmd` unless $noexec ==1;

                if($hiMem == 0){
                        $cmd = "samtools view -h $args{'bam'} | grep -Fvf $args{'root_dir'}/$bam_name.ids_to_rem.txt | samtools view -bS -> $args{'root_dir'}/$bam_name.noAbundant.bam ; samtools index $args{'root_dir'}/$bam_name.noAbundant.bam ; rm $args{'root_dir'}/$bam_name.ids_to_rem.txt";
                }
                else{
                        my $nb_line_split = (split(/ /,`wc -l $args{'root_dir'}/$bam_name.ids_to_rem.txt`))[0];
                        $nb_line_split = int($nb_line_split/2)+1;
                        system("split -d -l $nb_line_split $args{'root_dir'}/$bam_name.ids_to_rem.txt $args{'root_dir'}/$bam_name.ids_to_rem.");
                        $cmd = "samtools view -h $args{'bam'} | grep -Fvf $args{'root_dir'}/$bam_name.ids_to_rem.00 | samtools view -bS -> $args{'root_dir'}/$bam_name.noAbundant00.bam ; samtools index $args{'root_dir'}/$bam_name.noAbundant00.bam ; rm $args{'root_dir'}/$bam_name.ids_to_rem.{00,txt} ; samtools view -h $args{'root_dir'}/$bam_name.noAbundant00.bam | grep -Fvf $args{'root_dir'}/$bam_name.ids_to_rem.01 | samtools view -bS -> $args{'root_dir'}/$bam_name.noAbundant.bam ; samtools index $args{'root_dir'}/$bam_name.noAbundant.bam ; rm $args{'root_dir'}/$bam_name.ids_to_rem.01 ; rm $args{'root_dir'}/$bam_name.noAbundant00.bam $args{'root_dir'}/$bam_name.noAbundant00.bam.bai";
                }
                print "\t$cmd\n";
                print `$cmd` unless $noexec ==1;
...
if($rnaseq == 1){
	...
	if($norm ==1){
		print "####Create a copy for the normalization protocol as well\n";
	...
	if($um == 1){
		...
		if($norm == 1){
			print "####Create a copy for the normalization protocol as well\n";
			print "####Remove non-uniquely mapped reads\n";  ###MODIFY FOR STAR OR TOPHAT2
                	$cmd = "samtools view -@ $cpu -h $intermediate_dir/$bam_name_norm.bam | grep -P \"NH:i:1\\t|^@\" | samtools view -bS - > $intermediate_dir/$bam_name_norm.UM.bam ; samtools index $intermediate_dir/$bam_name_norm.UM.bam";
        	        print "\t$cmd\n";
	                print `$cmd` unless $noexec == 1;
			$bam_name_norm = "$bam_name_norm.UM";
		}
	...
	if($um == 1){
		print "####Remove non-uniquely mapped reads\n";
		if($casava == 1){
                        $cmd = "samtools view -@ $cpu -b -F256 $args{'bam'} > $intermediate_dir/$bam_name.UM.bam";
                }
                else{
                        $cmd = "samtools view -@ $cpu -h $args{'bam'} | grep -P \"XT:A:U\$|^@\" | samtools view -bS - > $intermediate_dir/$bam_name.UM.bam";
                }
	        print "\t$cmd\n";
	        print `$cmd` unless $noexec == 1;
...
if($indelRealign == 1){
	if($rnaseq!=1){
        	...
        	$cmd = "samtools view -@ $cpu -b -F4 $args{'bam'} > $intermediate_dir/$bam_name.onlyMapped.bam";
	        print "\t$cmd\n";
        	print `$cmd` unless $noexec == 1;
	...
	if($intervals eq 'NA'){
	       	$cmd = "$gatk_real -R $ref -T RealignerTargetCreator -o $intermediate_dir/$bam_name.intervals -I $args{'bam'} --validation_strictness LENIENT -DBQ 0";
		...
		$cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
	        ...
	        print "\t$cmd\n";
       		print `$cmd` unless $noexec == 1;
		if($norm ==1){
			$cmd = "$gatk_real -R $ref -T RealignerTargetCreator -o $intermediate_dir/$bam_name_norm.intervals -I $intermediate_dir/$bam_name_norm.bam --validation_strictness LENIENT -DBQ 0";
	                $intervals = "$intermediate_dir/$bam_name_norm.intervals";
        	        $cmd .= " -known $dbsnp" if $ref =~ /hg18_c/;
                	$cmd .= " -known $indel_1000G_138" if $ref =~ /GRCh37/;
	                $cmd .= " -known $indel_1000G_138" if $ref =~ /CEU/;
        	        $cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
	...
	my $cmd2;
	if($rnaseq == 1){
		$cmd = "$gatk_real -I $args{'bam'} -R $ref -T IndelRealigner -o $intermediate_dir/$bam_name.realignedGATK.bam -targetIntervals $intervals -known $indel_1000G --consensusDeterminationModel KNOWNS_ONLY -LOD 0.4 --validation_strictness LENIENT -U ALLOW_N_CIGAR_READS -DBQ 0";
		if($norm == 1){
			$cmd2 = "$gatk_real -I $intermediate_dir/$bam_name_norm.bam -R $ref -T IndelRealigner -o $intermediate_dir/$bam_name_norm.realignedGATK.bam -targetIntervals $intervals -known $indel_1000G --consensusDeterminationModel KNOWNS_ONLY -LOD 0.4 --validation_strictness LENIENT -U ALLOW_N_CIGAR_READS -DBQ 0";
		}
	}
	else{
	        $cmd = "$gatk_real -I $args{'bam'} -R $ref -T IndelRealigner -o $intermediate_dir/$bam_name.realignedGATK.bam -targetIntervals $intervals --consensusDeterminationModel KNOWNS_ONLY -LOD 0.4 --validation_strictness LENIENT -DBQ 0";
	}
	$cmd .= " -known $dbsnp" if $ref =~ /hg18_c/;
	...
	if($norm == 1){
		$cmd2 .= " -known $dbsnp" if $ref =~ /hg18_c/;
	        $cmd2 .= " -known $indel_1000G" if $ref =~ /hg19/;

		print "\t$cmd2\n";
	        print `$cmd2` unless $noexec == 1;
		$bam_name_norm = "$bam_name_norm.realignedGATK";
	}
...
if (!exists($args{'norecal'})) {
	...
	$cmd = "$gatk -T BaseRecalibrator -I $args{'bam'} -R $ref -knownSites $dbsnp -o $intermediate_dir/$bam_name.recal_data.table -nct $cpu -DBQ 0";
	$cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
	print "\t$cmd\n";
	print `$cmd` unless $noexec == 1;
	$cmd = "$gatk_real -T PrintReads -R $ref -I $args{'bam'} -BQSR $intermediate_dir/$bam_name.recal_data.table -o $intermediate_dir/$bam_name.recalibrate.bam -nct $cpu -DBQ 0";
	$cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
	print "\t$cmd\n";
	print `$cmd` unless $noexec == 1;
	if ( ($tibo == 1) && ($noexec != 1) ) {
	...
	if($norm ==1 ){
		$cmd = "$gatk -T BaseRecalibrator -I $intermediate_dir/$bam_name_norm.bam -R $ref -knownSites $dbsnp -o $intermediate_dir/$bam_name_norm.recal_data.table -nct $cpu -DBQ 0";
	        $cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
        	print "\t$cmd\n";
	        print `$cmd` unless $noexec == 1;
        	$cmd = "$gatk_real -T PrintReads -R $ref -I $intermediate_dir/$bam_name_norm.bam -BQSR $intermediate_dir/$bam_name_norm.recal_data.table -o $intermediate_dir/$bam_name_norm.recalibrate.bam -nct $cpu -DBQ 0";
	        $cmd .= " -U ALLOW_N_CIGAR_READS" if $rnaseq ==1;
        	print "\t$cmd\n";
	        print `$cmd` unless $noexec == 1;
        	if ( ($tibo == 1) && ($noexec != 1) ) {
	        ...
	        $bam_name_norm = "$bam_name_norm.recalibrate";
	}
...
if($trim == 1){
	...
	if($norm==1){
		$cmd = "$bamUtil trimBam $intermediate_dir/$bam_name_norm.bam $intermediate_dir/$bam_name_norm.trim2baseEnd.bam 2 ; samtools index $intermediate_dir/$bam_name_norm.trim2baseEnd.bam";
	        print "\t$cmd\n";
        	print `$cmd` unless $noexec == 1;
	        $args{'bam'} = "$intermediate_dir/$bam_name_norm.trim2baseEnd.bam";
        	$bam_name_norm = "$bam_name_norm.trim2baseEnd";
	}
}
...
if ( ($args{'calling'} eq 'snp') or ($args{'calling'} eq 'both') ) {
	...
	if($rnaseq!=1){	
		$cmd = "$gatk -pairHMM VECTOR_LOGLESS_CACHING -T HaplotypeCaller -I $args{'bam'} -R $ref --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o $output_dir/$bam_name.g.vcf --dbsnp $dbsnp -stand_call_conf 30.0 -stand_emit_conf 0.0 -L $kit_bed -dontUseSoftClippedBases -mmq 1 -DBQ 0";
	}
	else{
		$cmd = "$gatk -pairHMM VECTOR_LOGLESS_CACHING -T HaplotypeCaller -I $args{'bam'} -R $ref --emitRefConfidence GVCF --variant_index_type LINEAR --variant_index_parameter 128000 -o $output_dir/$bam_name.g.vcf --dbsnp $dbsnp -dontUseSoftClippedBases -stand_call_conf 20.0 -stand_emit_conf 20.0 -mmq 1 -U ALLOW_N_CIGAR_READS -DBQ 0";
	}
	print "$cmd\n";
               print `$cmd` unless $noexec == 1;
...
sub get_args{
	...
	foreach my $arg (@args) {
		if ($arg =~ /^-(\w+)$/ ) {
			...
			if(uc($current_arg) eq 'CLEAN'){
                                $result_args{'clean'}=1;
                        }
			if(uc($current_arg) eq 'NORM'){
                                $result_args{'norm'}=1;
                        }
